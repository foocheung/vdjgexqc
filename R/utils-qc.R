make_flow_tables <- function(vdj_list, seurat_filt, seurat_merged = NULL,
  ...
  paired_vdj <- barcode_summary |> dplyr::filter(paired_bcr)
  
  # ── Robust join: use config + barcode_clean when config is available on
  #    both sides; fall back to barcode_clean-only when GEX cell names don't
  #    carry a parseable config (e.g. names like "AG_neg_AAACCTGA...").
  join_vdj_gex <- function(vdj_tbl, gex_lookup, gex_col) {
    gex_has_config <- !all(is.na(gex_lookup$config))
    
    if (gex_has_config) {
      dplyr::left_join(vdj_tbl, gex_lookup, by = c("config", "barcode_clean"))
    } else {
      # Join on barcode only; drop config from lookup to avoid name clash
      dplyr::left_join(
        vdj_tbl,
        dplyr::select(gex_lookup, -config),
        by = "barcode_clean"
      )
    }
  }
  ...
  vdj_gex_match <- paired_vdj |>
    join_vdj_gex(filtered_lookup, "filtered_gex_cell") |>
    join_vdj_gex(merged_lookup,   "merged_gex_cell") |>
    dplyr::mutate(
  ...
  mapped_to_merged_gex           <- ifelse(is.null(seurat_merged), NA_integer_,
                                           sum(vdj_gex_match$maps_to_merged_gex))
      ...
      vdj_gex_match |>
        ...
        dplyr::summarise(
          ...
          percent_paired_mapped_to_filtered_gex  = round(
            paired_vdj_mapped_to_filtered_gex / dplyr::n() * 100, 2
          ),
...
make_barcode_loss_summary <- function(paired_vdj_barcodes, vdj_gex_match,
                                      seurat_merged = NULL) {
  ...
  if (is.null(seurat_merged)) {
    tibble::tibble(
      ...
      count = c(paired_vdj_barcodes, mapped_filtered, not_mapped_filtered),
      percent_of_paired_vdj = round(count / paired_vdj_barcodes * 100, 2)
    )
  } else {
    mapped_merged          <- sum(vdj_gex_match$maps_to_merged_gex)
    in_merged_not_filtered <- sum(vdj_gex_match$maps_to_merged_gex &
                                    !vdj_gex_match$maps_to_filtered_gex)
    ...
    tibble::tibble(
      ...
      count = c(paired_vdj_barcodes, mapped_merged, mapped_filtered,
                in_merged_not_filtered, not_in_merged),
      percent_of_paired_vdj = round(count / paired_vdj_barcodes * 100, 2)
    )
  }
}